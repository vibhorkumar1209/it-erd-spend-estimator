...
// Load data at cold-start (bundled with deployment)
const dataPath = join(process.cwd(), 'src', 'data.json');
const data = JSON.parse(readFileSync(dataPath, 'utf-8'));

type LookupData = typeof data;

function calculateSpend(companyName: string, revenue: number, industry: string, country: string) {
    ...
    etCats.forEach((name: any) => {
        ...
        if (name === 'AI (ML/DL/GenAI & Safety)' && erdHeavyIndustries.includes(industry)) {
            const aiErdBreakdown = erdBreakdown.find((b: any) => b.id === 'AI/ML & Data Engineering');
    ...
    return {
        companyName, revenue, industry, country, region, trends,
...
export default function handler(req: VercelRequest, res: VercelResponse) {
    ...
    if (!companyName || revenue === undefined || !industry || !country) {
        return res.status(400).json({
            ...
            example: { companyName: 'Equinor', revenue: 5000, industry: 'Energy (Oil & Gas)', country: 'Norway' }
        });
    }
